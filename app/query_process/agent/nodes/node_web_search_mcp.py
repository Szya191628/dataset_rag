...
async def mcp_call(query):
    ...
    # ==================================================================================
    # 初始化百炼MCP Streamable HTTP客户端
    # ----------------------------------------------------------------------------------
    # MCPServerStreamableHttp 是基于 Streamable HTTP 协议的 MCP 客户端实现。
    # 它的作用是连接到阿里云百炼提供的 MCP 服务端点，从而让我们可以像调用本地函数一样调用远程工具。
    #
    # 参数解释：
    # name: 客户端名称，用于日志标识，方便调试。
    # params: 连接配置字典
    #   - url: MCP 服务的接口地址 (例如: .../mcps/WebSearch/mcp)
...
if __name__ == '__main__':
    ...
    test_state = {
        ...
        "rewritten_query": "华为显示器B3-211H是什么？",
        "is_stream": False
    }
